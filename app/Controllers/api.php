...
class api extends BaseController
{
    ...
    public function login()
    {
        ...
        // Gunakan getGet() karena permintaan dari jQuery pakai $.get()
        $nik = $this->request->getGet("nik");
        $passwordInput = $this->request->getGet("password");

        $response = [];

        if ($nik && $passwordInput) {
                ...
                foreach ($user1->getResult() as $user) {
                    ...
                    if ($passwordInput === $decrypted) {
                        $response = [
                            ...
                            'password' => $passwordInput // hati-hati menyimpan password!
                        ];
                    } else {
                        $response = [
                            'status' => 'fail',
    ...
    public function createtarget()
    {
        ...
        $input["machineposition_name"] = $this->request->getGET("machineposition_name");

        // Persiapkan response dalam format JSON
        $response = [
            "status" => $status,
            "message" => $message,
            "data" => $target
